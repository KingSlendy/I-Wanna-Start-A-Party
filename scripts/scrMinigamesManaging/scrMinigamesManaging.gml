...
function minigame_init() {
	...
	m[$ "4vs"] = [
		...
		new Minigame(JINGLE_SLEDGE, [DESC_START draw_page("Rules", "Sledge in the snow couldn't be left out! But there's obstacles in the way! Shoot the trees and candies to destroy them and jump to avoid the spikes. First to reach the goal wins!") DESC_END, DESC_START draw_page("Controls", draw_action(global.actions.shoot) + " Shoot\n" + draw_action(global.actions.jump) + " Jump") DESC_END], 35, rMinigame4vs_Jingle, "Jingle Jam")
	];
