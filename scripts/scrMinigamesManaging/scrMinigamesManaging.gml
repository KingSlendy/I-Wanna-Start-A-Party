...
function minigame_init() {
	...
	m[$ "2vs2"] = [
		...
		new Minigame(JINGLE_SLEDGE, [DESC_START draw_page("Rules", "Sledge in the snow couldn't be left out! But there's obstacles in the way! The one at the front needs to shoot the trees and candies. The one behind needs to jump over the spikes. First to reach the goal wins!") DESC_END, DESC_START draw_page("Controls", draw_action(global.actions.shoot) + " Shoot (Front)\n" + draw_action(global.actions.jump) + " Jump (Behind)") DESC_END], 35, rMinigame2vs2_Jingle, "Jingle Jam")
	];
}
