...
function vm_group_arg_on_arg_index(l_th,l_act,l_scope,l_st){
	var l_z=l_st[0];
	var l_k=l_st[l_z];
	if(gml_std_gml_NativeTypeHelper_isNumber(l_k)){
		var l_i=(l_k|0);
		var l_args1=l_scope[3];
...
function vm_group_env_on_env1d(l_th,l_act,l_scope,l_st){
	...
	var l_z=l_st[0];
	var l_i=l_st[l_z];
	if(gml_std_gml_NativeTypeHelper_isNumber(l_i))l_st[@l_z]=l_fn(false,undefined,(l_i|0)); else return l_th.h_proc_error2("Index "+gml_value_print(l_i)+" is not a number.",l_act);
	return 0;
}
...
function vm_group_env_on_env1d_set(l_th,l_act,l_scope,l_st){
	...
	l_st[@l_z]=0;
	l_st[@l_z+1]=0;
	if(gml_std_gml_NativeTypeHelper_isNumber(l_i))l_fn(true,l_v,(l_i|0)); else return l_th.h_proc_error2("Index "+gml_value_print(l_i)+" is not a number.",l_act);
	return 0;
}
...
function vm_group_env_on_env1d_aop(l_th,l_act,l_scope,l_st){
	...
	l_st[@l_z]=0;
	l_st[@l_z+1]=0;
	if(!gml_std_gml_NativeTypeHelper_isNumber(l_i))return l_th.h_proc_error2("Index "+gml_value_print(l_i)+" is not a number.",l_act);
...
function vm_group_field_dump_no_var(l_th,l_act,l_ctx,l_fd){
	...
	if(gml_std_gml_NativeTypeHelper_isNumber(l_ctx)){
	...
	if(gml_std_gml_NativeTypeHelper_isNumber(l_ctx)&&l_ctx>=100000){
		...
		with (l_ctx) {
			...
			if(l_cur!=undefined||variable_instance_exists(l_ctx,l_s)){
				...
				l_checkExists=true;
			} else if(gml_std_gml_NativeTypeHelper_isNumber(l_ctx)){
				l_st[@l_z]=0;
				l_st[@l_z+1]=0;
				return l_th.h_proc_error2("`"+gml_std_Std_stringify(l_ctx)+"` ("+object_get_name(l_ctx.object_index)+") does not have a variable `"+l_s+"`",l_act);
			} else {
...
function vm_group_field_on_self_field(l_th,l_act,l_scope,l_st){
	...
	if(l_val==undefined){
		if(!variable_struct_exists(l_obj,l__field)){
			if(gml_std_gml_NativeTypeHelper_isNumber(l_obj)){
...
function vm_group_field_on_local_field_set(l_th,l_act,l_scope,l_st){
	l_scope[4][l_act.h_localIndex][$ l_act.h_fd]=gml_stack_pop(l_st);
	return 0;
}
...
function vm_group_field_on_in(l_th,l_act,l_scope,l_st){
	...
	if(gml_std_gml_NativeTypeHelper_isNumber(l_ctx)&&l_ctx>=100000){
		l_st[@l_z]=(l_not!=variable_instance_exists(l_ctx,l_st[l_z])?1:0);
		return 0;
	} else with (l_ctx) {
		l_st[@l_z]=(l_not!=variable_instance_exists(self,l_st[l_z])?1:0);
		return 0;
	}
	if(gml_std_gml_NativeTypeHelper_isNumber(l_ctx)&&object_exists(l_ctx))return l_th.h_proc_error2("Couldn't find any instances of "+gml_std_Std_stringify(l_ctx)+" ("+object_get_name(l_ctx)+")",l_act); else return l_th.h_proc_error2("Couldn't find any instances of "+gml_std_Std_stringify(l_ctx)+".",l_act);
	return 0;
...
function vm_group_field_on_in_const(l_th,l_act,l_scope,l_st){
	var l_z=l_st[0];
	l_st[@l_z]=(l_act.h_not!=variable_struct_exists(l_st[l_z],l_act.h_field)?1:0);
	return 0;
}
...
function vm_group_jump_on_repeat_pre(l_th,l_act,l_scope,l_st){
	var l_p=l_act.h_p;
	var l_val=l_st[l_st[0]];
	if(gml_std_gml_NativeTypeHelper_isNumber(l_val)){
		if(l_val<1){
			l_scope[@2]=l_p;
			var l_i=l_st[0];
			l_st[@l_i]=0;
...
function vm_group_op_on_equ_op(l_th,l_act,l_scope,l_st){
	var l_z=l_st[0]-1;
	l_st[@0]=l_z;
	l_st[@l_z]=(l_st[l_z]==l_st[l_z+1]?1:0);
	l_st[@l_z+1]=0;
	return 0;
}
...
function vm_group_special_on_wait(l_th,l_act,l_scope,l_st){
	...
	l_th.h_status=5;
	l_th.h_wait_time=(gml_std_gml_NativeTypeHelper_isNumber(l_t)?l_t:0);
