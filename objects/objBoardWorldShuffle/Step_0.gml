if (state == 0) {
	...
	if (fade_alpha >= 1) {
		...
		with (objBoardWorldGhost) {
			...
			with (objSpaces) {
				if (image_index == SpaceType.Red || image_index == SpaceType.ChanceTime || image_index == SpaceType.TheGuy || image_index == SpaceType.Surprise && !place_meeting(x, y, objPlayerBase)) {
