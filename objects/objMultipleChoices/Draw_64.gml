...
if (array_length(titles) > 0) {
	...
	draw_set_halign(fa_center);
	draw_set_valign(fa_middle);
	//var text = new Text(global.fntDialogue, titles[global.choice_selected]);
...
if (array_length(descriptions) > 0) {
	...
	var text = new Text(global.fntDialogue, descriptions[global.choice_selected]);
...
for (var i = 0; i < array_length(choices); i++) {
	...
	var text = new Text(global.fntDialogue, ((is_selected) ? "{SWIRL}" : "") + choices[i]);
