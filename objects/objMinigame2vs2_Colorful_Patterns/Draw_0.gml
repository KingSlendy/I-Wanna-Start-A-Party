for (var i = 0; i < 4; i++) {
	draw_sprite(sprMinigame2vs2_Colorful_Round, (i < pattern_round), x + 75 * i, y + 32 * (pattern_rows + 1));
}

if (array_length(pattern_grid) == 0) {
	exit;
}
...
if (pattern_player_ids != null) {
	for (var i = 0; i < 2; i++) {
		if (trial_is_title(COLORFUL_MADNESS) && i == 1 && !pattern_selected[0]) {
			continue;
		}
