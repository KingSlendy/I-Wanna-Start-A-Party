		...
		case SpaceType.Shine:
			if (room != rBoardHyrule || global.board_light) {
				...
				if (player_info.coins >= global.shine_price) {
					...
					start_dialogue([
						new Message(language_get_text("PARTY_BOARD_SHINE_NOT_ENOUGH", ["{X coins}", draw_coins_price(global.shine_price)]),, board_advance)
					]);
...
function space_finish_event() {
	...
	switch (image_index) {
		...
		case SpaceType.Surprise:
			switch (irandom(4)) {
				case 0: change_coins(space_give * 5, CoinChangeType.Gain).final_action = turn_next; break;
