...
if ((self_pokemon.power_type == other_pokemon.power_type && irandom(1) == 0) || win_types[$ self_pokemon.power_type] == other_pokemon.power_type) {
	with (other_pokemon) {
		if (place_meeting(x, y - 32, objShine)) {
			change_shines(1, ShineChangeType.Get).final_action = board_advance;
		} else {
			change_coins(20, CoinChangeType.Gain).final_action = board_advance;
		}
	}
} else {
	change_coins(-5, CoinChangeType.Lose).final_action = board_advance;
}
